...
def test_update_component_builds_update_request(monkeypatch):
    """Ensure update requests include the explicit wiki credentials.
    ...
    client.update_component("Q123", "primary", b"hello", media_type="application/pdf", username="testuser", password="testpass")

    request = captured["request"]
    assert request.header.op_code == OP_UPDATE
    assert request.object_id == "Q123"
    assert request.metadata_blocks == [{"operation": "update", "element": "primary", "username": "testuser", "password": "testpass"}]
...
def test_update_component_reads_credentials_from_environment(monkeypatch):
    """Ensure update requests fall back to the environment for credentials.
    ...
    monkeypatch.setenv("DOIP_USERNAME", "env-user")
    monkeypatch.setenv("DOIP_PASSWORD", "env-pass")
    monkeypatch.setattr(StrictDOIPClient, "send_message", fake_send_message)
    ...
    request = captured["request"]
    assert request.metadata_blocks == [{"operation": "update", "element": "primary", "username": "env-user", "password": "env-pass"}]
