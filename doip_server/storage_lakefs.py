...
def _bucket() -> str:
    """Return bucket/repository name for lakeFS-backed storage.
...
async def list_components(object_id: str) -> List[str]:
    ...
    result: List[str] = []
    async for page in _async_paginate(paginator, Bucket=_bucket(), Prefix=prefix):
...
async def component_metadata(object_id: str, component_id: str) -> Dict[str, str]:
    ...
    """
    key = s3_key_from_component(object_id, component_id)
    head = await asyncio.to_thread(_client().head_object, Bucket=_bucket(), Key=key)
    return {
